...
### nping使用手冊與語法
```
Nping 0.5.36TEST2 ( http://nmap.org/nping )
Usage: nping [Probe mode] [Options] {target specification}

TARGET SPECIFICATION:測試目標的設定
  Targets may be specified as hostnames主機名稱, IP addresses, networks, etc.
  Ex: scanme.nmap.org, microsoft.com/24, 192.168.0.1; 10.0.0-255.1-254

PROBE MODES:使用不同協定的測試模式
  --tcp-connect                    : Unprivileged(無特權) TCP connect probe mode.
  --tcp                            : TCP probe mode.
  --udp                            : UDP probe mode.
...
IPv4 OPTIONS:
  ...
  --ip-options <hex string>                    : Set IP options small enough.
...
ECHO CLIENT/SERVER:
  ...
  --echo-client <passphrase>       : Run Nping in server mode.
